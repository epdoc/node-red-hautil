...
function setFan(gHA, fnSend, params, opts) {
  ...
  const DELAY = (0, _epdocUtil.isNonEmptyArray)(opts.delay) ? opts.delay : [1000, 3000];
  const ha = new _ha.HA(gHA);
  const bShutoff = (0, _epdocUtil.isNonEmptyString)(opts.shutoff_entity_id) ? ha.entity('input_boolean.lightning').isOn() : false;
  ...
  if ((0, _epdocUtil.isInteger)(params.speed)) {
    ...
    speed = _.Service.fanPercentageToSpeed(params.percentage);
  ...
  return Promise.resolve().then(resp => {
    ...
    if (swutch.isOn() && (bShutoff || bOff || !bOn && speed === 0)) {
      log(`Turn off ${fan_id}`);
      let payload = (0, _.newFanSpeed6Service)(params.fan).off().payload();
      fnSend(payload);
    } else {
    ...
    if (!swutch.isOn() && !bShutoff && (bOn || speed > 0)) {
      ...
      let payload = (0, _.newSwitchService)(switch_id).on().payload();
      fnSend(payload);
      bTurnedOn = true;
    } else {
    ...
    if (!bShutoff && speed > 0 && bTurnedOn) {
      ...
      return (0, _epdocUtil.delayPromise)(DELAY[0]);
    } else {
    ...
    if (!bShutoff && speed > 0) {
      ...
      let payload = (0, _.newFanSpeed6Service)(params.fan).speed(speed).payload();
      fnSend(payload);
      log('2nd delay of ' + DELAY[1] + ' for ' + switch_id);
      return (0, _epdocUtil.delayPromise)(DELAY[1]);
    } else {
    ...
    if ((bOn || speed > 0) && timeout && !bShutoff) {
      ...
      return (0, _epdocUtil.delayPromise)(timeout);
    } else {
      return Promise.resolve();
    }
  }).then(function () {
    if ((bOn || speed > 0) && timeout && !bShutoff) {
      ...
      let payload = (0, _.newSwitchService)(switch_id).off().payload();
      fnSend(payload);
    }
    return Promise.resolve();
  });
